...
const page = require('../controller/movies.js');
...
describe(`OMDB Movie List`, () => {
 ...
 it(`@get ${testCase.positive.getList}`, async() => {
  const response = await page.search(apiKey, keySearch);
  assert(response.status).to.equal(200);
 }),

 it(`@get ${testCase.negative.noSearch}`, async() => {
  const response = await page.search(apiKey, '');
 ...
 it(`@get ${testCase.negative.invalidApiKey}`, async() => {
   const response = await page.search(invalidApiKey, keySearch);
